...
describe('GET /account/users', () => {
  it ('should return an unverified token error', done => {
...
config.test.free &&
describe('POST /api/user/verify/request', () => {
  it ('should re-send the verification email and return a verification token', done => {

    chai.request(server)
    .post('/api/user/verify/request')
    .set(config.auth, process.env.free_token)
    .send(config.account.free)
    .end((err, res) => {

      res.should.have.status(200);
      res.body.verification_token.should.be.a('string');
      process.env.verification_token = res.body.verification_token;
      done();
...
describe('POST /api/user/verify', () => {
  it ('verify the account', done => {
    ...
    .set(config.auth, process.env.free_token)
    .send({ ...config.account.free, ...{ token: process.env.verification_token }})
    .end((err, res) => {

      res.should.have.status(200);
      res.body.token.should.be.a('string');
      process.env.free_token = 'Bearer ' + res.body.token;
      done();
...
describe('POST /api/user/verify', () => {
  it ('verify the account and return a new token', done => {

    chai.request(server)
    .post('/api/user/verify')
    .set(config.auth, process.env.token)
    .send({ ...config.account.paid, ...{ token: process.env.verification_token }})
    .end((err, res) => {
